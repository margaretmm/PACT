...
public class ConsumerClient {
    ...
    public String postBody(String path, String body, ContentType mimeType) throws IOException {
        Request a = Request.Post(url + encodePath(path))
                .bodyString(body, mimeType);
        System.out.println(a);
        return a.execute().returnContent().asString();
    }

    public String postBody(String path, String host, String body, ContentType mimeType) throws IOException {
        Request a = Request.Post(url + encodePath(path))
                .bodyString(body, mimeType).setHeader("HOST",host);
        System.out.println(a);
        Response resp = a.execute();
//        InputStream b = resp.returnResponse().getEntity().getContent();
//        System.out.println(b);
        String c = resp.returnContent().asString();
        System.out.println(c);
        return c;
    }

    public int postBodyStatus(String path, String host, String body, ContentType mimeType) throws IOException {
        Request a = Request.Post(url + encodePath(path))
                .bodyString(body, mimeType).setHeader("HOST",host);
        System.out.println(a);
        return a.execute().returnResponse().getStatusLine().getStatusCode();
    }
